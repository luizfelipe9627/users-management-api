...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    const updateQuery = `UPDATE users SET ${fieldsToUpdate.join(
      ", ",
    )} WHERE id = ?`; // A constante updateQuery armazena a consulta SQL que será feita no banco de dados para atualizar o usuário.
    ...
    const [updatedUsers] = await connection.execute(updatedUserQuery, [
      user.id,
    ]); // A constante updatedUsers armazena o resultado da consulta ao banco de dados.
    ...
    return res
      ...
      }); // Retorna uma resposta com status 200 e um objeto JSON com as propriedades name, email e password do usuário atualizado.
  } catch (err) {
    return res
      .status(500)
      .json({
