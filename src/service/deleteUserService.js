...
const deleteUser = async (req, res) => {
    ...
    try {
        ...
        if (comparePassword) {
            ...
            return res.status(400).json({ error: 'A senha fornecida está incorreta. Verifique e tente novamente.' }); // Retorna uma resposta com status 400 e um objeto JSON com a propriedade error e a mensagem de erro.
        }
    } catch (err) {
        return res.status(500).json({ error: 'Erro ao deletar usuário. Por favor, tente novamente mais tarde.' }); // Retorna uma resposta com status 500 e um objeto JSON com a propriedade error e a mensagem de erro.
    }
}
