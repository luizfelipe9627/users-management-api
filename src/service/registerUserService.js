...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    const query =
      "INSERT INTO users (name, email, password, token) VALUES (?,?,?,?);"; // A constante query armazena a consulta SQL que será feita no banco de dados para inserir um novo usuário.
    // O await faz com que a execução da função seja pausada até que a Promise seja resolvida, ou seja, até que a senha seja validada.
    const [result] = await connection.execute(query, [
      ...
      hashPassword,
      token,
    ]); // A constante result armazena o resultado da consulta ao banco de dados,
