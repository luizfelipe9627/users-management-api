...
const validateRegister = async (req, res, next) => {
    ...
    try {
        ...
        if(await emailExists(email)) {
            return res.status(400).json({ error: 'O endereço de e-mail já está em uso.'}); // Retorna um erro 400 com a mensagem de que o e-mail já está em uso.
        }
...
const validateLogin = async (req, res, next) => {
    ...
    try {
        ...
        if(!await emailExists(email)) {
            return res.status(400).json({ error: 'O endereço de e-mail digitado não existe.'}) // Retorna um erro 400 com a mensagem de que o e-mail não existe.
        }
